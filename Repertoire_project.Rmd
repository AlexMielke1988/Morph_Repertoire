...
Some gesture actions are not meant to include repetition but were coded that way by some students. This is removed here.

```{r remove_repetition}
# Define vector of gestures to set Repetition_count to NA
na_gestures <- c('LocomoteGallop', 'SpinPirouette', 'SpinRoulade', 'SpinSomersault')

# Loop over clean.data and clean.data.reduced data frames to set Repetition_count to NA for na_gestures
for (gesture in na_gestures) {
  clean.data$Repetition_count[clean.data$Gesture_record == gesture] <- NA
  clean.data.reduced$Repetition_count[clean.data.reduced$Gesture_record == gesture] <- NA
}
...
# Update Repetition column
clean.data.reduced$Repetition <- ifelse(clean.data.reduced$Repetition_count > 0, "yes", "no")

# Update Laterality column
clean.data.reduced$Laterality <- ifelse(clean.data.reduced$Laterality_signaller %in% c("Left", "Right"), "unimanual", clean.data.reduced$Laterality_signaller)
...
# Replace values in Body_part_contact column
clean.data.reduced$Body_part_contact <- case_when(
  clean.data.reduced$Body_part_contact == 'Fist' ~ 'Fingers_Hand',
  clean.data.reduced$Body_part_contact %in% c('BodyChest', 'BodyFront') ~ 'Body',
  clean.data.reduced$Body_part_contact == 'Bottom' ~ 'Leg',
  TRUE ~ clean.data.reduced$Body_part_contact
)

# Replace values in Body_part_signaller column
clean.data.reduced$Body_part_signaller <- case_when(
  clean.data.reduced$Body_part_signaller %in% c('Fist', 'Knuckles') ~ 'Hand',
  clean.data.reduced$Body_part_signaller %in% c('BodyChest', 'BodyFront') ~ 'Body',
  clean.data.reduced$Body_part_signaller == 'HandFoot' ~ 'Foot',
  TRUE ~ clean.data.reduced$Body_part_signaller
)
